...
void PrintHeaderService(Printer* printer, const Service* service,
    ...
    printer->Print("/* 设置消息目的地，默认发送到SCENE节点 */\n");
    printer->Print("void SetNode(::pebble::RpcNode node);\n\n");
#ifndef __RPC_CLIENT__
...
void PrintSourceClientMethod(Printer* printer, const Method* method,
    ...
    if (is_public) {
        ...
        printer->Print("__head.m_node = m_imp->m_node > 0 ? m_imp->m_node : __head.m_node;\n");
...
void PrintSourceService(Printer* printer, const Service* service,
    ...
    printer->Print(*vars, "void $Service$Client::SetNode(::pebble::RpcNode node) {\n");
    printer->Indent();
    printer->Print("m_imp->m_node = node;\n");
    printer->Outdent();
    printer->Print("}\n\n");
#ifndef __RPC_CLIENT__
