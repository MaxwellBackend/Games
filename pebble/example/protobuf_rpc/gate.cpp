...
int main(int argc, const char** argv) {
    ...
    std::cout << "bind handle:" << handle << std::endl;
    ...
    pebble::PebbleRpc* pebble_rpc = server.GetPebbleRpc(pebble::kPEBBLE_RPC_PROTOBUF);
    ASSERT_EQ(true, rpc != NULL);
    std::string service_address("tcp://127.0.0.1:9001");
    int64_t handle2 = server.Connect(service_address);
    std::cout << "connect handle:" << handle2 << std::endl;
    if (handle < 0) {
            PLOG_ERROR("connect failed, handle: %ld, service_address: %s",
                            handle, service_address.c_str());
            return 0;
    }
    server.Attach(handle2, pebble_rpc);

    rpc->AddNodeFunction(0x03,handle2);
